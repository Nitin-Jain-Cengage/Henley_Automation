 ...
 */
public class MTXInstructorTakeDeleteTest {
	...
	public void TC001_loginToTheSSOFrontDoor(
			@Optional("chemistry") String courseType) {
		test.loginActionsPg.loginToTheApplication(
				getYamlValue("users.instructor.username"),
				getYamlValue("users.instructor.password"));
		bookType = courseType.toLowerCase().trim();
	}
	...
	public void TC002_performOperationsOnHomePage() {
		test.instTakeDeletePg.selectProdIsbn(getYamlValue("DeleteTake."
				+ bookType + ".isbn"));
		test.instTakeDeletePg.clickOnCourseName(getYamlValue("DeleteTake."
				+ bookType + ".coursename"), getYamlValue("testenv"));
	}
	...
	public void TC004_actionsOnStudentTab() {
		...
		test.instTakeDeletePg.clickOnStudent(getYamlValue("DeleteTake."
				+ bookType + ".studentname"));
	}
